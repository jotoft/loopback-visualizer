...
int main()
{
    ...
    float x_scale = (float)fb_width / (float)window_width;
    float y_scale = (float)fb_height / (float)window_height;
    gui_state.dpi_scale = (x_scale + y_scale) * 0.5f;
    
    std::cout << "Window size: " << width << "x900, Framebuffer: " << fb_width << "x" << fb_height << std::endl;
    std::cout << "DPI scale: " << gui_state.dpi_scale << std::endl;
    ...
    // Configure ImGui for HiDPI
    ImGuiStyle& style = ImGui::GetStyle();
    style.ScaleAllSizes(gui_state.dpi_scale);
    io.FontGlobalScale = gui_state.dpi_scale;
    
    // Load larger font for HiDPI
    ImFontConfig font_config;
    font_config.SizePixels = 13.0f * gui_state.dpi_scale;
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        // ImGui rendering
        {
            ...
            ImGui::SetNextWindowSize(ImVec2(350 * gui_state.dpi_scale, 500 * gui_state.dpi_scale), ImGuiCond_FirstUseEver);
            ...
            if (ImGui::CollapsingHeader("Audio Devices", ImGuiTreeNodeFlags_DefaultOpen)) {
                ...
                for (size_t i = 0; i < app_state.available_devices.size(); ++i) {
                    ...
                    if (ImGui::Selectable(label.c_str(), is_selected)) {
                        app_state.selected_device_idx = i;
            ...
            if (ImGui::CollapsingHeader("UI Settings")) {
                if (ImGui::SliderFloat("UI Scale", &gui_state.dpi_scale, 0.5f, 3.0f)) {
                    ImGuiStyle& style = ImGui::GetStyle();
                    style = ImGuiStyle(); // Reset to default
                    style.ScaleAllSizes(gui_state.dpi_scale);
                    ImGui::GetIO().FontGlobalScale = gui_state.dpi_scale;
                }
            }
            ...
            if (gui_state.show_correlation_graph && gui_state.phase_lock_enabled) {
                ImGui::SetNextWindowPos(ImVec2(370 * gui_state.dpi_scale, 10), ImGuiCond_FirstUseEver);
                ImGui::SetNextWindowSize(ImVec2(400 * gui_state.dpi_scale, 200 * gui_state.dpi_scale), ImGuiCond_FirstUseEver);
                ...
                if (!gui_state.correlation_history.empty()) {
                    ...
                    ImGui::PlotLines("Correlation", values.data(), values.size(), 
                                     0, nullptr, 0.0f, 1.0f, ImVec2(0, 150 * gui_state.dpi_scale));
                }
